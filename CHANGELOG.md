...
- **Typed env substitution restored for config values.** 0.4.1 moved `${VAR}` substitution after JSON parsing, which broke previously-working unquoted references like `"maxRetries": ${TL_RETRIES}`. A quoted `"${VAR}"` on a number or boolean field now converts to that type, and the parse error for the unquoted form explains the migration. Conversion is driven by the field's declared type, not by the env value's shape, so a numeric-looking value on a string field (`"model": "${TL_MODEL}"` with `TL_MODEL=2`) stays a string instead of failing with `CONFIG_INVALID`.
