...
export function makeGlossaryView(state: AppState, parent: BoxRenderable): View {
  const { renderer, glossaryStore, config } = state;
  ...
  const fromPicker = makeLangPicker(renderer, "g-from-picker", config.defaults?.sourceLang ?? "en", false);
  ...
  const toPicker = makeLangPicker(renderer, "g-to-picker", config.defaults?.targetLang ?? "ar", false);
